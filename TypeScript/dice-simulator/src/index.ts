...
// GUI要素を取得
const diceCountInput = document.getElementById("dice-count") as HTMLInputElement;
...
const resultArea = document.getElementById("results") as HTMLDivElement;
 ...
 * グラフを作成
 */
function createChart(results: number[]) {
  const frequencies = results.reduce((acc: { [key: number]: number }, result) => {
 ...
 * サイコロの要素を作成
 */
function createDiceElements(count: number): HTMLDivElement[] {
  diceContainer.innerHTML = "";
  const diceElements: HTMLDivElement[] = [];
 ...
 * サイコロの目を表示
 */
function showDiceFaces(diceElements: HTMLDivElement[], results: number[], critical: number) {
  diceElements.forEach((diceElement, index) => {
    ...
    faces.forEach((face, faceIndex) => {
      ...
      (face as HTMLElement).style.color = isCritical ? "red" : "";
 ...
 * 結果と履歴を更新
 */
function updateResults(results: number[], critical: number) {
